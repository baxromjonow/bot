...
export async function sendNextQuiz() {
  ...
  const groups = await select(
    "groups",
    "active=eq.true&select=chat_id,title&order=added_at.asc"
  );
    ...
    try {
      ...
      await insert(
        "quiz_deliveries",
        {
          ...
          telegram_message_id: pollMessage.message_id,
          success: true,
          error: null
        },
  ...
  await update("quizzes", `id=eq.${quiz.id}`, {
    status: "sent",
  ...
  return {
    ok: true,
    message: "Quiz yuborish yakunlandi",
    quizId: quiz.id,
    sent,
    failed,
    errors
