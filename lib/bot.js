...
async function showQueue(chatId) {
  const parts = [];
  for (const track of SUBJECT_IDS) {
    const rows = await select("quizzes", `subject=eq.${track}&select=question,category,quiz_no&order=quiz_no.asc&limit=20`);
    const countRows = await select("quizzes", `subject=eq.${track}&select=id`);
    const total = countRows?.length || 0;
    parts.push(`<b>${subjectLabel(track)}</b> — <b>${total} ta</b>`);
    if (rows?.length) {
