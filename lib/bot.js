...
import { insert, remove, rpc, select, update } from "./supabase.js";
...
Guruh ichida:
/guruh — Quizy mavzusini guruhga ulash
...
async function sendNextSpecial(chatId) {
  ...
  const groups=await select("groups","active=eq.true&select=chat_id,title,quiz_topic_id");
  if(!groups?.length) return sendMessage(chatId,"Faol guruh yo‘q.");
  const { sendQuiz }=await import("./telegram.js");
  let sent=0,failed=0;
  for(const group of groups){
    try{ if(!group.quiz_topic_id){ failed++; continue; } await sendQuiz(group.chat_id,quiz,group.quiz_topic_id); sent++; }
...
async function importExcel(chatId, adminId, document, track) {
  ...
  const prepared = quizzes.map((q, i) => ({
    question: q.question,
    options: q.options,
    correct_option: q.correct_option,
    explanation: q.explanation || "",
    category: q.category || "Excel",
    quiz_no: i + 1,
    track: track === "javascript" ? "javascript" : (track === "html_css" ? "html_css" : "computer"),
    fingerprint: trackFingerprint(track, q.question)
  }));

  const result = await rpc("replace_subject_quizzes", { p_subject: track, p_quizzes: prepared });
  const replacedCount = Number(Array.isArray(result) ? result[0] : result) || prepared.length;
  await clearState(adminId); await refreshLowQueueAlertState();
  return sendMessage(chatId, `✅ <b>${subjectLabel(track)}</b> savollar bazasi almashtirildi.

🧠 Yangi quizlar: <b>${replacedCount} ta</b>
♻️ Eski ${subjectLabel(track)} quizlari o‘chirildi.
👥 Guruhlarning fan va x/72 progressi saqlandi.`);
}
...
async function handlePrivateAdminMessage(message) {
  ...
  switch (state.step) {
    ...
    case "explanation": {
      const explanation=text==="-"?null:text; const rows=await insert("quizzes",{question:data.question,options:data.options,correct_option:data.correct_option,explanation,category:"Qo‘lda",subject:data.track,track:(data.track==="javascript"?"javascript":(data.track==="html_css"?"html_css":"computer")),fingerprint:trackFingerprint(data.track,data.question),status:"queued"},{ignoreDuplicates:true,onConflict:"fingerprint"});
...
async function handleCallback(callback) {
  ...
  // Guruhni ulash callbacklari alohida himoyalanadi: faqat /guruh yuborgan guruh admini.
  if (a.startsWith("connectchange:") || a.startsWith("connectsubject:") || a.startsWith("connectlesson:") || a.startsWith("connectcancel:")) {
    ...
    if (String(callback.from.id) !== ownerId) {
      return answerCallbackQuery(callback.id, "🔒 Bu tanlovni /guruh yuborgan admin qiladi.");
    }
  ...
  if(a.startsWith("connectlesson:")){
    ...
    const legacyTrack=subject==="javascript"?"javascript":(subject==="html_css"?"html_css":"computer");
    const topicId = Number(callback.message.message_thread_id || 0);
    await insert("groups",{chat_id:chatId,title,track:legacyTrack,subject,quiz_position:quizPosition,quiz_topic_id:topicId || null,progress_updated_at:new Date().toISOString(),active:true},{upsert:true,onConflict:"chat_id"});
  ...
  if(a.startsWith("groupchoose:")){ return sendMessage(chatId,"Progressni o‘zgartirish uchun guruhning Quizy mavzusida /guruh buyrug‘idan foydalaning."); }
...
async function handleGroupMessage(message) {
  ...
  if(!["/guruh","/removegroup"].includes(command)) return;
  ...
  if(command==="/guruh") {
    ...
    if (!message.is_topic_message || !topicId) {
      return sendMessage(chatId,"⛔ <b>/guruh</b> buyrug‘ini guruhdagi <b>Quizy</b> mavzusi ichida yuboring. Bot quizlarni faqat shu mavzuga jo‘natadi.");
    }
    const existing=await select("groups",`chat_id=eq.${chatId}&select=chat_id,title,track,subject,quiz_position,active,quiz_topic_id&limit=1`);
    const group=existing?.[0];
    const ownerId=String(message.from.id);
    if(group?.active && group?.subject && isSubject(group.subject)) {
      await update("groups",`chat_id=eq.${chatId}`,{quiz_topic_id:topicId,title});
      return sendMessage(chatId,`ℹ️ <b>${escapeHtml(title)}</b> allaqachon botga ulangan.\n\n🧠 Quizy mavzusi yangilandi.\nHozirgi holat: <b>${subjectLabel(group.subject)} — ${group.quiz_position || 1}/72</b>\n\nFan/darsni qayta belgilamoqchimisiz?`,{message_thread_id:topicId,reply_markup:{inline_keyboard:[[{text:"⚙️ Almashtirish",callback_data:`connectchange:${ownerId}`}],[{text:"❌ Bekor qilish",callback_data:`connectcancel:${ownerId}`}]]}});
    }
    await update("groups",`chat_id=eq.${chatId}`,{quiz_topic_id:topicId,title});
    return sendMessage(chatId,`🧠 <b>Quizy mavzusi ulandi.</b>\n\n📚 <b>${escapeHtml(title)}</b> hozir qaysi fanda?`,{message_thread_id:topicId,reply_markup:subjectKeyboard("connectsubject",ownerId)});
  }
