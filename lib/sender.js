...
export async function sendNextQuiz(subject = null) {
  ...
  const groups = await select("groups", `active=eq.true${filter}&select=chat_id,title,subject,quiz_position,last_quiz_id,last_quiz_subject,last_quiz_no,quiz_topic_id&order=added_at.asc`);
  if (!groups?.length) {
  ...
  for (const group of groups) {
    ...
    try {
      if (!group.quiz_topic_id) { failed++; continue; }
      const poll = await sendQuiz(group.chat_id, quiz, group.quiz_topic_id);
