...
async function sendNow(chatId) {
  await sendMessage(chatId, "⚡ Qabul qilindi. Keyingi quiz yuborilmoqda…");
...
async function handlePrivateAdminMessage(message) {
  ...
  if (command === "/start" || command === "/help") {
    await clearState(adminId);
    await sendMessage(chatId, HELP, { reply_markup: REPLY_MENU });
    return showPanel(chatId);
  }
  if (["🎛 menu", "menu"].includes(text.toLowerCase())) return showPanel(chatId);
  if (text === "📊 Statistika") return showStats(chatId);
  if (text === "📥 Excel") {
    await setState(adminId, "excel_file", {});
    return sendMessage(chatId, "Excel <b>.xlsx</b> faylini yuboring. Kategoriya ustuni ixtiyoriy.");
  }
  if (text === "📋 Navbat") return showQueue(chatId);
  if (text === "🚀 Hozir yuborish") return sendNow(chatId);
  if (text === "👥 Guruhlar") return showGroups(chatId);
  if (text === "⚙️ Sozlamalar") return showSettings(chatId);
  if (text === "💾 Backup") return sendBackup(chatId);
  ...
  if (!state) return sendMessage(chatId, HELP, { reply_markup: REPLY_MENU });
