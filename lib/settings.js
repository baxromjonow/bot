...
export async function getSchedule({ fresh = false } = {}) {
  const now = Date.now();
  if (!fresh && scheduleCache && now < scheduleCacheUntil) return scheduleCache;
  ...
  scheduleCacheUntil = now + CACHE_TTL_MS;
  return schedule;
}
...
export async function updateSchedule(patch) {
  ...
  scheduleCache = next;
  scheduleCacheUntil = Date.now() + CACHE_TTL_MS;
  return next;
}

export function clearScheduleCache() {
  scheduleCache = null;
  scheduleCacheUntil = 0;
}
