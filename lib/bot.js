...
async function showSpecialMenu(chatId) {
  const queued = await select("special_quizzes", "status=eq.queued&select=id");
  return sendMessage(chatId, `⚡ <b>Maxsus quizlar</b>\n\n📋 Navbatda: <b>${queued?.length || 0} ta</b>\n\nBu quizlar faqat siz yuborganda barcha faol guruhlarga ketadi. Asosiy fan/dars progressiga ta’sir qilmaydi.`, { reply_markup: specialKeyboard() });
}

async function showSpecialQueue(chatId) {
  const rows = await select("special_quizzes", "status=eq.queued&select=id,question,created_at&order=id.asc&limit=20");
  if (!rows?.length) return sendMessage(chatId, "📋 <b>Maxsus navbat bo‘sh.</b>");
  const lines = rows.map((q,i)=>`${i+1}. ${escapeHtml(q.question)}`);
  return sendMessage(chatId, `📋 <b>Maxsus navbat</b>\n\n${lines.join("\n")}\n\n${rows.length===20?"<i>Birinchi 20 tasi ko‘rsatildi.</i>":""}`);
}
...
async function sendNextSpecial(chatId) {
  ...
  return sendMessage(chatId,`⚡ <b>Maxsus quiz yuborildi.</b>\n\n👥 Yuborildi: <b>${sent}</b>\n❌ Xato: <b>${failed}</b>\n\nAsosiy guruh progresslari o‘zgarmadi.`);
}

async function clearSpecialQueue(chatId) {
  const deleted=await remove("special_quizzes","status=eq.queued");
  return sendMessage(chatId,`🗑 Maxsus navbat tozalandi. <b>${deleted?.length||0} ta</b> yuborilmagan quiz o‘chirildi.`);
}
...
async function handleCallback(callback) {
  ...
  if(a==="panel:special") return showSpecialMenu(chatId);
  if(a==="special:excel") return beginSpecialExcel(chatId,adminId);
  if(a==="special:queue") return showSpecialQueue(chatId);
  if(a==="special:send") return sendMessage(chatId,`🚀 <b>Navbatdagi maxsus quiz barcha faol guruhlarga yuborilsinmi?</b>\n\nBu asosiy 72 talik progressga ta’sir qilmaydi.`,{reply_markup:confirmKeyboard("specialsend")});
  if(a==="specialsend:yes") return sendNextSpecial(chatId);
  if(a==="specialsend:no") return sendMessage(chatId,"❌ Maxsus yuborish bekor qilindi.");
  if(a==="special:clear") return sendMessage(chatId,"⚠️ <b>Yuborilmagan barcha maxsus quizlar o‘chirilsinmi?</b>",{reply_markup:confirmKeyboard("specialclear")});
  if(a==="specialclear:yes") return clearSpecialQueue(chatId);
  if(a==="specialclear:no") return sendMessage(chatId,"❌ Tozalash bekor qilindi.");
