...
async function handleCallback(callback) {
  ...
  if(a.startsWith("connecttrack:")){const track=a.split(":")[1];if(!isTrack(track))return;const title=callback.message.chat.title||`Guruh ${chatId}`;await insert("groups",{chat_id:chatId,title,track,active:true},{upsert:true,onConflict:"chat_id"});return sendMessage(chatId,`✅ <b>${escapeHtml(title)}</b> botga ulandi.\nYo‘nalish: <b>${trackLabel(track)}</b>`);}
...
async function handleGroupMessage(message) {
  ...
  if(command==="/connect") return sendMessage(chatId,`📚 <b>${escapeHtml(title)}</b> qaysi yo‘nalishda?`,{reply_markup:trackKeyboard("connecttrack")});
