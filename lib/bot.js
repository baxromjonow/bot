...
import { TRACK_IDS, isTrack, trackKeyboard, trackLabel, trackShort } from "./tracks.js";
...
Guruh ichida:
/register — guruh yo‘nalishini tanlab ro‘yxatga olish
...
async function handleCallback(callback) {
  ...
  if(a.startsWith("registertrack:")){const track=a.split(":")[1];if(!isTrack(track))return;const title=callback.message.chat.title||`Guruh ${chatId}`;await insert("groups",{chat_id:chatId,title,track,active:true},{upsert:true,onConflict:"chat_id"});return sendMessage(chatId,`✅ <b>${escapeHtml(title)}</b> ro‘yxatga olindi.\nYo‘nalish: <b>${trackLabel(track)}</b>`);}
...
async function handleGroupMessage(message) {
  ...
  if(command==="/register") return sendMessage(chatId,`📚 <b>${escapeHtml(title)}</b> qaysi yo‘nalishda?`,{reply_markup:trackKeyboard("registertrack")});
