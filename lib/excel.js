...
const HEADERS = {
  ...
  category: ["kategoriya", "category", "fan", "subject"]
};
...
function validateQuiz(rowNumber, values) {
  ...
  const categoryRaw = asText(values.category);
  const category = categoryRaw || "Boshqa";
  ...
  if (category.length > 80) errors.push("Kategoriya 80 belgidan uzun.");

  if (errors.length) return { ok: false, rowNumber, errors };
  ...
  return {
    ...
    quiz: {
      ...
      explanation,
      category,
      fingerprint: quizFingerprint(question),
      status: "queued"
    }
  };
}
...
export async function parseQuizWorkbook(buffer) {
  ...
  for (const [key, aliases] of Object.entries(HEADERS)) {
    ...
    if (index === -1 && !["explanation", "category"].includes(key)) {
      throw new Error(`Excel ustuni topilmadi: ${key}. Namuna fayldagi sarlavhalarni o‘zgartirmang.`);
    }
    columns[key] = index;
  }
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    const values = {
      ...
      category: columns.category === -1 ? null : row[columns.category]
    };
  ...
  if (!quizzes.length && !validationErrors.length) throw new Error("Excel faylda quizlar topilmadi.");
